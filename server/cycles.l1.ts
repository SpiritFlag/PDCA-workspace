// Design Ref: §8.4 D-39·D-48 — cycles L1 하네스. `app.request()`로 미들웨어→라우트→서비스→scoped→
...
// 골격(mintToken·seedWorkspaceProject·cleanup 등)은 `./l1-harness.js`로 추출됐다(6차 D-48) —
//   `server/mcp.l1.ts`가 같은 골격을 가져다 쓴다. 이 파일은 cycles 시나리오만 남는다.
import { randomUUID } from 'node:crypto'
import { afterAll, beforeAll, describe, expect, it } from 'vitest'
import { body, cleanup, errBody, hasDb, mintToken, req, seedWorkspaceProject } from './l1-harness.js'
