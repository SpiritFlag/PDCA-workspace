...
| **WHO** | 형 — 브라우저 실사용자이자 승인자, **브라우저 실증의 실행 주체**(4T-1) / 클로드(Claude Code CLI) — 코드·dev DB 하네스 / claude.ai 웹 MCP 커넥터 — **이번 스코프에서는 소비자가 아니다**(cycles MCP 툴 0개, F36) |
