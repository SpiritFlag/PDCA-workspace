...
| **WHO** | 형 — 브라우저 실사용자. 이번엔 클로드가 소비자가 아니다(cycles MCP 툴 0개, 누락). |
...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|-----------|--------|
| 0.1 | 2026-08-08 | **사후 재구성** 최초 작성. 구현된 구조를 설계 형식으로 기술하고, 규약을 어긴 자리 8곳을 ⚠️로 표시. §3.3(zod 결함 2건)·§3.2(마이그레이션 0002→0003 화석)·§5.3(ImportDialog 규약 위반 3건)·§8.1(런타임 실증 0건)이 본 문서의 실질 산출물 | Claude |
