...
| **WHO** | 형 — 승인자이자 **프로덕션 배포·웹 커넥터 실호출의 실행 주체**(4T-1) / 클로드(Claude Code CLI) — 코드·dev DB 하네스 / **claude.ai 웹 MCP 커넥터 — 이번 사이클의 1급 소비자**(4·5차와 정반대. C21·C23·C25의 판정 주체가 웹 커넥터 실호출이다) |
