...
> **(사후 2026-08-08)**: 이월된 Critical·Important·Minor 3건 + C7·C10 전부 refine-mcp-hardening
> 사이클에서 해소·완주됨 — 상세는 §1.4·§4.1·§4.2
>
> **프로젝트**: PDCA-workspace
> **작성자**: Claude
> **완료일**: 2026-08-07
...
**(사후 2026-08-08 확인, refine-mcp-hardening 사이클 S6)**: 위 박스는 2026-08-07 Report 작성
시점의 스냅샷이다. 그 뒤 형이 claude.ai 웹 MCP 커넥터로 **C7·C10을 완주**해 미완료 2건이
해소됐다(§1.4 상세). 이 박스 자체는 수정하지 않는다 — 그 시점의 판단 기록이라는 의미가 있다.
...
| C6 | Claude Code 원격 MCP `tools/list`에 8개 | ⚠️ Partial | 프로토콜 레벨(`app.request()`) 완전 실증. **실제 `claude mcp add` 연결은 미실행** — **(사후 2026-08-08 확인, refine-mcp-hardening M-1/S6)** claude.ai 웹 커넥터로는 연결·`tools/call` 다건 실사용 확인됨(F27). **Claude Code CLI의 `claude mcp add --transport http` 연결은 여전히 미검증** — 웹 기준 충족·CLI 기준 미검증으로 구분 |
| C7 | 유즈케이스 3단계 완주(이 사이클의 관문) | ~~❌ Not Met~~ → **✅ Met (사후)** | 배포 + 실제 Claude Code 세션 필요 — 형이 배포 후 진행. **(사후 2026-08-08 확인)** claude.ai 웹 MCP 커넥터로 완주됨 — Claude Code CLI가 아니라 웹 클라이언트로 달성(F27, refine-mcp-hardening S6) |
| C8 | `backlog_update`로 doing/done 거부 | ✅ Met | 실 DB L1 #6 — zod 1차 방어에서 거부 확인 |
| C9 | 미인증 MCP 401 | ✅ Met | 실 DB L1 #1 |
| C10 | 형이 실제로 이월 16건을 넣고 사용 | ~~❌ Not Met~~ → **✅ Met (사후)** | 미실행 — 배포 후 형이 직접. **(사후 2026-08-08 확인)** claude.ai 웹 MCP 커넥터로 완주됨(F27, refine-mcp-hardening S6) |
...
| 항목 | 사유 | 우선순위 |
|------|------|:---:|
| ~~처리일(closedOn) 삭제 불가~~ | Critical이지만 형이 "다른 사이클에서 실사용하며 검증" 선택. **(사후 2026-08-08 해소, refine-mcp-hardening S1/C11)** `.nullable()` + UI ✕ 버튼으로 삭제 가능해짐 — API+브라우저 실증 완료 | ~~중~~ |
| ~~MCP Origin 거부 에러코드가 §6.1 표 밖~~ | Important, 기능엔 지장 없음(403은 정확히 남). **(사후 2026-08-08 해소, S2/C12)** `FORBIDDEN`(403) 신설 + `ServiceError` 경유로 표 정합화 | ~~중~~ |
| ~~MCP 툴 zod 스키마가 `shared/schema.ts` 미재사용~~ | Minor, 날짜 포맷 등에서 REST보다 느슨할 수 있음(구체 사례 미확인). **(사후 2026-08-08 해소, S4/C14)** 갭 16곳 필드별 재사용으로 폐쇄, 구체 사례(`openedOn` 포맷)로 확정 후 수정 — 웹 커넥터 실사용으로 무회귀 확인 | ~~낮음~~ |
| 종료 상태 전환 시 처리일 "제안"이 실제로는 즉시 확정 저장 | Minor | 낮음 |
| PAT 진입이 "사용자 메뉴"가 아니라 상시 헤더 링크 | Minor | 낮음 |
| OAuth 2.1 정식 지원 | claude.ai 웹의 근본 해법. D-18(쿼리토큰)로 임시 대응 중. **(사후 2026-08-08, refine-mcp-hardening S3)** `?token=`이 Vercel 로그에 평문으로 남는 것을 실측 확인(C13) — 승격 근거가 늘었으나 이번에도 착수는 보류(형 결정) | 낮음(필요해지면 승격) |

### 4.2 배포·실사용 필요 (C7·C10) — **(사후 2026-08-08 전부 해소)**

| 항목 | 필요 조건 | 상태 |
|------|-----------|:---:|
| C7 — 실제 Claude Code 세션 유즈케이스 3단계 완주 | main 배포 + `claude mcp add --transport http` 연결 + 형이 실제 세션에서 확인 | ~~미완~~ → **✅ 완주(웹 커넥터, F27)** |
| C10 — 1차 사이클 이월 16건을 형이 실제로 백로그에 입력 | 배포 후 형이 직접 | ~~미완~~ → **✅ 완료(웹 커넥터, F27)** |

두 항목 다 필요조건이 명시한 "Claude Code 세션"이 아니라 **claude.ai 웹 MCP 커넥터**로
달성됐다 — 클라이언트 종류를 좁혀 적었던 게 실제로는 넓게 열려 있었다(2차 회고 §6.2 Problem 3
"클라이언트 종류를 스코프 질문에 안 넣음"과 같은 축의 교훈).
...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|-----------|--------|
| 0.1 | 2026-08-07 | 완료 보고서 최초 작성. Match Rate 96%, Success Rate 3/10(부분 5, 미충족 2 — 전부 형의 브라우저·배포 실사용이 필요한 항목) | Claude |
| 0.2 | 2026-08-08 | **(사후 확인, refine-mcp-hardening 사이클 S6)** C7·C10을 claude.ai 웹 MCP 커넥터로 완주(F27) — §헤더·§1.2·§1.4·§4.2 반영. §4.1에서 Critical(처리일 삭제 불가)·Important(Origin 에러코드)·Minor(MCP 스키마 미재사용) 3건 해소 반영. 원문은 삭제하지 않고 취소선+사후 문구로 병기(D-25) | Claude |
