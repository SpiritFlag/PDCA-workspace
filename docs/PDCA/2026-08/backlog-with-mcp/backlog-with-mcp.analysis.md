...
| C6 | `tools/list`에 툴 8개 | ⚠️ | 프로토콜 레벨(`app.request()` 시뮬레이션)로는 완전 실증(L1 #10). **실제 `claude mcp add` 연결은 미실행** |
| C7 | 유즈케이스 3단계 완주 (관문) | ❌ | 배포 + 실제 Claude Code 세션 필요. 이번 세션에서 미실행 |
| C8 | `backlog_update`로 doing/done 거부 | ✅ | L1 #6 실증(zod 1차 방어에서 거부) |
| C9 | 미인증 MCP 401 | ✅ | L1 #1 실증 |
| C10 | 형이 실제로 이월 16건을 넣고 사용 | ❌ | 아직 미실행 |
